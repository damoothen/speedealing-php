...
Source0: http://www.speedealing.com/files/lastbuild/package_rpm_generic/%{name}-%{version}.tgz
...
Speedealing was designed to provide only features you need and be easy to 
use.
...
Speedealing ha sido diseñado para suministrarle solamente las funcionalidades
...
# Create install.forced.php into Speedealing install directory
...
# Show result
echo
echo "----- Speedealing %version-%release - (c) Speedealing dev team -----"
echo "Speedealing files are now installed (into /usr/share/dolibarr)."
echo "To finish installation and use Speedealing, click on the menu" 
echo "entry Speedealing ERP-CRM or call the following page from your"
echo "web browser:"  
...
if [ -L $apachelink ] ;
then
    echo "Delete apache config link for Speedealing ($apachelink)"
    %{__rm} -f $apachelink
    status=purge
fi
