 ...
 * Copyright (C) 2012		Regis Houssin        <herve.prot@symeos.com>
 *
...
if ($action == 'set') {
	...
	if (!$error && GETPOST("filename")) {
		...
		if ($file) {
			...
			if (!$error)
				$res = dolibarr_set_const($db, "SYSLOG_FILE_ON", $syslog_file_on, 'chaine', 0, '', 0);
		}
		else {
			$error++;
	...
	if (!$error && GETPOST("facility")) {
		...
		if (defined($_POST["facility"])) {
			...
			if (!$error)
				$res = dolibarr_set_const($db, "SYSLOG_SYSLOG_ON", $syslog_syslog_on, 'chaine', 0, '', 0);
		}
		else {
			$error++;
	...
	if (!$error && isset($_POST['SYSLOG_FIREPHP_ON'])) {	// If firephp no available, post is not present
		...
		if (!$error)
			$res = dolibarr_set_const($db, "SYSLOG_FIREPHP_ON", $syslog_firephp_on, 'chaine', 0, '', 0);
	}
...
print '<td align="right" colspan="2"><input type="submit" class="button nice tiny" ' . $option . ' value="' . $langs->trans("Modify") . '"></td>';
print "</tr>\n";
$var = true;
...
try {
	...
	if ($res) {
		...
		if (!class_exists('FirePHP'))
			...
			print ($syslog_firephp_on ? ' checked="checked"' : "");
...
print '<td align="right"><input type="submit" class="button nice tiny" ' . $option . ' value="' . $langs->trans("Modify") . '"></td>';
print "</tr>\n";
$var = true;
$var = !$var;
