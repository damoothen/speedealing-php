...
$admin = array(
		...
		'ConfirmPurgeSessions' => 'あなたは本当にすべてのセッションをパージしたいですか？これは（自分を除く）すべてのユーザーが切断されます。',
		...
		'PurgeDeleteTemporaryFiles' => 'すべての一時ファイル（ルーズのデータ​​へのリスクなし）を削除',
		...
		'InstrucToEncodePass' => '<b>conf.php</b>ファイルにエンコードされたパスワードを持っているために、次の行を置き換える<br> <b>$ dolibarr_main_db_pass = &quot;...&quot;</b> <br>によって<br> <b>$ dolibarr_main_db_pass = &quot;暗号化：%s&quot;</b>',
		'InstrucToClearPass' => '<b>conf.php</b>ファイルにパスワードデコードされた（クリア）するには、行を置き換えます。 <br> <b>dolibarr_main_db_pass $は= &quot;暗号化され...&quot;</b> <br>によって<br> <b>$ dolibarr_main_db_pass = &quot;%s&quot;</b>',
		...
		'GenericMaskCodes2' => '<b>{CCCC}</b>クライアントコード<br>グローバル·カウンタのオフセットとゼロ化せずにn個の文字で<b>{cccc000}</b>クライアントコードは、クライアントのrefカウンタが続いている。 <br> <b>{TTTT}</b> n文字の会社のタイプのコード（dictionnary -会社の種類を参照してください）。 <br>',
		...
		'GenericMaskCodes5' => '<b>ABCは{YY} {ミリメートル} - {000000} ABC0701-000099</b>与える<br> <b>{0000 100}-ZZZ / {DD} / XXXは0199-ZZZ/31/XXXを</b>与える',
		...
		'Permission75' => 'メンバー用のセットアップ·タイプと属性',
		...
		'Permission113' => 'セットアップfinancielアカウント（管理、カテゴリを作成する）',
		'Permission114' => 'トランザクションを統合する',
		...
		'BackupDesc2' => '*（あなたは、例えば、zipファイルを作ることができる）は、すべてのアップロードされ、生成されたファイルが含まれるドキュメントディレクトリ<b>（%s）</b>の内容を保存します。',
		...
		'RestoreDesc2' => '*新しいDolibarrインストールのドキュメントディレクトリまたはdirectoyこの現在の文書<b>（%s）</b>にファイルのツリーを抽出するディレクトリのドキュメントのアーカイブファイル（たとえばzipファイル）をリストアします。',
		...
		'SomeTranslationAreUncomplete' => 'いくつかの言語は、部分的に翻訳されるか、またはエラーが含まれて可能性があります。あなたには、いくつかのを検出した場合は、ディレクトリ<b>の</b> htdocs <b>/ langs</b>に<b>。langの</b>テキストファイルを修正してからフォーラムにそれらを提出することができます<a href="http://www.dolibarr.org/forum" target="_blank">http://www.dolibarr.org</a> 。',
		...
		'SendmailOptionMayHurtBuggedMTA' => '&quot;PHPのメール直接&quot;メソッドを使用してメールを送信する機能が正常にいくつかの受信メールサーバーによって解析されないかもしれないメールメッセージを生成します。その結果、いくつかのメールがthoose盗聴プラットフォームでホストされている人々によって読み取ることができないということです。これは、いくつかのインターネットプロバイダ（：フランスのオレンジ例）の場合です。これはDolibarrにもPHPにはなく、メールサーバーを受信した上に問題はありません。ただしセットアップに1にオプションMAIN_FIX_FOR_BUGGED_MTAを追加することができます - これを避けるためにDolibarrを変更する他の。ただし、他​​のサーバーにその点で厳密にSMTP規格に問題が発生することがあります。他のソリューション（おすすめし）がない欠点を持っていないメソッド &quot;SMTPソケットライブラリ&quot;を使用することです。',
		...
		'WebCalTestOk' => 'ユーザー \'%s\'成功したデータベース &quot;%s&quot;のサーバ &quot;%s&quot;への接続を確立します。',
		...
		'ConfirmDeleteProductLineAbility' => 'フォーム内の​​製品ラインを削除する確認',
		...
		'UseSearchToSelectProduct' => '（代わりにリストボックスを使用しての）製品を選択するには検索フォームを使用しています。 <br>あなたは製品を大量に（&gt; 100 000）がある場合にも、セットアップ - &gt;その他1に定数PRODUCT_DONOTSEARCH_ANYWHEREを設定することにより、速度を上げることができます。検索は、文字列の先頭に制限されます。',
		...
		'SendingsAbility' => '顧客配達のためのサポートsendingsシート',
		...
		'FreeLegalTextOnShippings' => 'shippings無料テキスト',
		////////// Deliveries //////////
		...
		'FCKeditorForProductDetails' => 'すべてのエンティティ（提案、受注、請求書、等..）の製品の詳細ラインの作成​​/版をWYSIWIGエディタ<font class="warning">警告：このケースではこのオプションを使用するには、PDFを作成するときに、それが特殊文字とページのフォーマッティングの問題を作成できるように真剣におすすめしていませんファイル。</font>',
		...
		'SummaryOfVatExigibilityUsedByDefault' => '選びましたオプションに従って、デフォルトでは付加価値税exigibilityの時間：',
		'OnDelivery' => '着払い',
		'OnPayment' => '支払いに',
		'OnInvoice' => '請求書',
		...
		'PathToGeoIPMaxmindCountryDataFile' => '国への変換Maxmind IPアドレスを含むファイルへのパス。 <br>例：は/ usr / local / share /のGeoIP / GeoIP.dat',
