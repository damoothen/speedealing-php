...
$errors = array(
		...
		'ErrorUserCannotBeDelete' => 'Пользователь не может быть удален. Может быть, это связано в Speedealing образований.',
		'ErrorFieldsRequired' => 'Некоторые обязательные поля не были заполнены.',
		'ErrorFailedToCreateDir' => 'Не удалось создать каталог. Убедитесь, что веб-сервер пользователь имеет разрешения на запись в каталог Speedealing документы. Если параметр <b>safe_mode</b> включен по этому PHP, проверьте, что Speedealing PHP файлы принадлежат к веб-серверу пользователей (или группы).',
		...
		'ErrorLDAPSetupNotComplete' => 'Speedealing-LDAP соответствия не является полной.',
		...
		'ErrorDatabaseParameterWrong' => 'База данных установки параметра <b>&quot;%s&quot;</b> имеет значение не совместимы с использованием Speedealing (должно иметь значение <b>%s).</b>',
		...
		'ErrorForbidden2' => 'Разрешение на этот логин может быть определено администратором вашей Speedealing из меню %s-> %s.',
		'ErrorForbidden3' => 'Кажется, что Speedealing не используется через аутентифицированных сессии. Взгляните на Speedealing Настройка документации знать, как управлять подлинности (htaccess, mod_auth или другие ...).',
