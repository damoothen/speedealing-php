...
$errors = array(
		...
		'ErrorUserCannotBeDelete' => '用户不能被删除。也许它是相关联的Speedealing实体。',
		'ErrorFieldsRequired' => '一些必要的栏位都没有填补。',
		'ErrorFailedToCreateDir' => '无法创建一个目录。检查Web服务器的用户有权限写入Speedealing文件目录。如果参数<b>safe_mode设置</b>为启用这个PHP，检查Speedealing php文件到Web服务器的用户拥有（或组）。',
		...
		'ErrorLDAPSetupNotComplete' => 'Speedealing - LDAP的匹配是不完整的。',
		...
		'ErrorDatabaseParameterWrong' => '数据库设置参数<b>\'％s\'</b>的值不兼容使用Speedealing（必须有值<b>\'％s\'）。</b>',
		...
		'ErrorProductWithRefNotExist' => '参考<i>“%s”</i>产品不存在',
		...
		'ErrorForbidden2' => '此登录权限可以定义你的Speedealing从菜单％的S ->％s的管理员',
		'ErrorForbidden3' => '看来Speedealing是不是通过身份验证的会话中使用。以在Speedealing安装文件就会知道如何管理认证（htaccess的，mod_auth或其他...).',
