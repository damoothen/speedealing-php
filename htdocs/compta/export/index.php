...
$updir = $conf->comptabilite->dir_output."/export/";
$dir = $conf->comptabilite->dir_output."/export/".$year."/";
...
if ($_GET["action"] == 'export')
{
  ...
  if($_GET["id"] > 0)
    {
      if($_GET["type"] == 'rebuild') {
      $exc->Export($_GET["id"], $dir, 'rebuild');
      }  else {
          $exc->Export($_GET["id"], $dir, 'new');
      }
    ...
    {
      if($_GET["type"] == 'rebuild') {
      $exc->Export(0, $dir, 'rebuild');
      }else {
          $exc->Export(0, $dir, 'new');
      }
    }
