...
function dol_print_address($address, $htmlid, $mode, $id) {
	...
	if ($address) {
		...
		if ($mode == 'thirdparty' && $conf->google->enabled && $conf->global->GOOGLE_ENABLE_GMAPS)
			$showgmap = 1;
		if ($mode == 'contact' && $conf->google->enabled && $conf->global->GOOGLE_ENABLE_GMAPS_CONTACTS)
			$showgmap = 1;
		if ($mode == 'member' && $conf->google->enabled && $conf->global->GOOGLE_ENABLE_GMAPS_MEMBERS)
			$showgmap = 1;
		if ($mode == 'thirdparty' && $conf->openstreetmap->enabled && $conf->global->OPENSTREETMAP_ENABLE_MAPS)
			$showomap = 1;
		if ($mode == 'contact' && $conf->openstreetmap->enabled && $conf->global->OPENSTREETMAP_ENABLE_MAPS_CONTACTS)
			$showomap = 1;
		if ($mode == 'member' && $conf->openstreetmap->enabled && $conf->global->OPENSTREETMAP_ENABLE_MAPS_MEMBERS)
			$showomap = 1;
...
function img_object($alt, $picto, $options = '', $pictoisfullpath = 0) {
	...
	// Clean parameters
	if (!preg_match('/(\.png|\.gif)$/i', $picto) && !preg_match('/^([^@]+)@([^@]+)$/i', $picto))
		$picto.='.png';
	...
	if (!empty($pictoisfullpath))
		...
		if (defined('DOL_URL_ROOT_ALT') && DOL_URL_ROOT_ALT && !file_exists(DOL_DOCUMENT_ROOT . '/' . $path . '/img/object_' . $picto))
...
function img_picto_common($alt, $picto, $options = '', $pictoisfullpath = 0) {
	...
	if (!preg_match('/(\.png|\.gif)$/i', $picto))
		$picto.='.png';
	if ($pictoisfullpath)
		return '<img src="' . $picto . '" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '"' . ($options ? ' ' . $options : '') . '>';
	if (!empty($conf->global->MAIN_MODULE_CAN_OVERWRITE_COMMONICONS) && file_exists(DOL_DOCUMENT_ROOT . '/theme/' . $conf->theme . '/img/' . $picto))
...
function img_help($usehelpcursor = 1, $usealttitle = 1) {
	...
	if ($usealttitle) {
		if (is_string($usealttitle))
			$s.=' alt="' . dol_escape_htmltag($usealttitle) . '" title="' . dol_escape_htmltag($usealttitle) . '"';
		else
			$s.=' alt="' . $langs->trans("Info") . '" title="' . $langs->trans("Info") . '"';
	}
	else
		$s.=' alt=""';
	$s.='>';
...
function img_down($alt = "default", $selected = 0) {
	...
	if ($selected)
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1downarrow_selected.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '" class="imgdown">';
	else
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1downarrow.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '" class="imgdown">';
}
...
function img_up($alt = "default", $selected = 0) {
	...
	if ($alt == "default")
		$alt = $langs->trans("Up");
	if ($selected)
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1uparrow_selected.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '" class="imgup">';
	else
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1uparrow.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '" class="imgup">';
}
...
function img_left($alt = "default", $selected = 0) {
	...
	if ($selected)
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1leftarrow_selected.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '">';
	else
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1leftarrow.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '">';
}
...
function img_right($alt = "default", $selected = 0) {
	...
	if ($selected)
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1rightarrow_selected.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '">';
	else
		return '<img src="' . DOL_URL_ROOT . '/theme/' . $conf->theme . '/img/1rightarrow.png" border="0" alt="' . dol_escape_htmltag($alt) . '" title="' . dol_escape_htmltag($alt) . '">';
}
...
function get_localtax($tva, $local, $societe_acheteuse = "") {
	...
	if ($local == 1 && !$mysoc->localtax1_assuj)
		return 0;
	...
	if (is_object($societe_acheteuse)) {
		if ($code_pays != $societe_acheteuse->pays_code)
			return 0;
		if ($local == 1 && !$societe_acheteuse->localtax1_assuj)
			return 0;
		elseif ($local == 2 && !$societe_acheteuse->localtax2_assuj)
			return 0;
	}
...
function get_default_localtax($societe_vendeuse, $societe_acheteuse, $local, $idprod = 0) {
	...
	if ($societe_vendeuse->country_id == 'ES' || $societe_vendeuse->country_code == 'ES') {
		...
		if ($idprod)
			return get_product_localtax_for_country($idprod, $local, $societe_vendeuse->country_code);
		else
			return -1;
	}
	return 0;
...
function picto_from_langcode($codelang) {
	...
	if (!empty($codelang)) {
		if ($codelang == 'auto')
			$ret = img_picto('', DOL_URL_ROOT . '/theme/common/flags/int.png', '', 1);
		else {
			//print $codelang;
			$langtocountryflag = array('ar_AR' => '', 'ca_ES' => 'catalonia', 'da_DA' => 'dk', 'fr_CA' => 'mq', 'sv_SV' => 'se');
			$tmpcode = '';
			if (isset($langtocountryflag[$codelang]))
				$tmpcode = $langtocountryflag[$codelang];
			else {
				$tmparray = explode('_', $codelang);
				$tmpcode = empty($tmparray[1]) ? $tmparray[0] : $tmparray[1];
			}
			if ($tmpcode)
				$ret.=img_picto($codelang, DOL_URL_ROOT . '/theme/common/flags/' . strtolower($tmpcode) . '.png', '', 1);
		}
	}
	return $ret;
}
