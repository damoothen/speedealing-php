...
class modGravatar extends DolibarrModules
{
	...
	function modGravatar($db)
	{
		...
		$this->values->numero = 2700;
		// Key text used to identify module (for permissions, menus, etc...)
		$this->values->rights_class = 'gravatar';
		...
		$this->values->description = "Use online Gravatar service (www.gravatar.com) to show photo of users/members (found with their emails).<br>Need an internet access.";
		...
		$this->values->phpmin = array(4,3);					// Minimum version of PHP required by module
		$this->values->need_dolibarr_version = array(2,7);	// Minimum version of Dolibarr required by module
		$this->values->langfiles = array();
		...
		// Example:
		// $this->values->rights[$r][0] = 2000; 				// Permission id (must not be already used)
		// $this->values->rights[$r][1] = 'Permision label';	// Permission label
		// $this->values->rights[$r][3] = 1; 					// Permission by default for new user (0/1)
		// $this->values->rights[$r][4] = 'level1';				// In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
		// $this->values->rights[$r][5] = 'level2';				// In php code, permission will be checked by test if ($user->rights->permkey->level1->level2)
		// $r++;
		...
		// $this->values->menu[$r]=array(	'fk_menu'=>'r=0',		// Use r=value where r is index key used for the parent menu entry (higher parent must be a top menu entry)
		...
		// $this->values->menu[$r]=array(	'fk_menu'=>'r=1',		// Use r=value where r is index key used for the parent menu entry (higher parent must be a top menu entry)
		//							'type'=>'left',			// This is a Left menu entry
		...
		// $this->values->export_sql_end[$r]  =' FROM ('.MAIN_DB_PREFIX.'facture as f, '.MAIN_DB_PREFIX.'facturedet as fd, '.MAIN_DB_PREFIX.'societe as s)';
	...
	{
		return $this->values->_load_tables('/mymodule/sql/');
	}
}
