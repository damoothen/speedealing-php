  ...
  $.widget( "filament.table", { // need to come up with a better namespace var...
    ...
    _create: function() {
      ...
      hdrCols.each(function(i){
         ...
         if ( !th.is("." + o.persist) ) {
	         var toggle = $('<li><input type="checkbox" name="toggle-cols" id="toggle-'+o.idprefix+''+i+'" value="'+id+'" /> <label for="toggle-'+o.idprefix+''+i+'">'+th.text()+'</label></li>');
      ...
      if (!o.checkContainer) {
         var menuWrapper = $('<div class="table-menu-wrapper" />'),
               menuBtn = $('<a href="#" class="gh_button small pill">Show/Hide columns</a>');
