...
class Propal extends CommonObject {
    ...
    function Propal($DB, $socid = "", $propalid = 0) {
        ...
        $this->labelstatut[5] = ($conf->global->PROPAL_STATUS_VALIDATED_LABEL ? $conf->global->PROPAL_STATUS_VALIDATED_LABEL : $langs->trans("PropalStatusPoss"));
        ...
        $this->labelstatut_short[1] = ($conf->global->PROPAL_STATUS_VALIDATEDSHORT_LABEL ? $conf->global->PROPAL_STATUS_VALIDATEDSHORT_LABEL : $langs->trans("PropalStatusOpenedShort"));
        ...
        $this->labelstatut_short[5] = ($conf->global->PROPAL_STATUS_VALIDATEDSHORT_LABEL ? $conf->global->PROPAL_STATUS_VALIDATEDSHORT_LABEL : $langs->trans("PropalStatusPossShort"));
    }
            ...
            if ($this->id) {
                ...
                if (!$error) {
                    ...
                    for ($i = 0; $i < $num; $i++) {
                        ...
                        $result = $this->addline(
                                $this->id, $this->lines[$i]->desc, $this->lines[$i]->subprice, $this->lines[$i]->qty, $this->lines[$i]->tva_tx, $this->lines[$i]->localtax1_tx, $this->lines[$i]->localtax2_tx, $this->lines[$i]->fk_product, $this->lines[$i]->remise_percent, 'HT', 0, 0, $this->lines[$i]->product_type, $this->lines[$i]->ecotax, $this->lines[$i]->rang, $this->lines[$i]->special_code, $fk_parent_line
                        );
    ...
    function LibStatut($statut, $mode = 1) {
        ...
        if ($mode == 2) {
            ...
            if ($statut == 1)
                return img_picto($langs->trans('PropalStatusOpenedShort'), 'statut2') . ' ' . $this->labelstatut_short[$statut];
            if ($statut == 2)
                return img_picto($langs->trans('PropalStatusSignedShort'), 'statut4') . ' ' . $this->labelstatut_short[$statut];
            if ($statut == 3)
            ...
            if ($statut == 5)
                return img_picto($langs->trans('PropalStatusPossShort'), 'statut3') . ' ' . $this->labelstatut_short[$statut];
        }
        if ($mode == 3) {
            if ($statut == 0)
                return img_picto($langs->trans('PropalStatusDraftShort'), 'statut0');
            if ($statut == 1)
                return img_picto($langs->trans('PropalStatusOpenedShort'), 'statut2');
            if ($statut == 2)
                return img_picto($langs->trans('PropalStatusSignedShort'), 'statut4');
            if ($statut == 3)
            ...
            if ($statut == 5)
                return img_picto($langs->trans('PropalStatusPossShort'), 'statut3');
        }
        if ($mode == 4) {
            if ($statut == 0)
                return img_picto($langs->trans('PropalStatusDraft'), 'statut0') . ' ' . $this->labelstatut[$statut];
            if ($statut == 1)
                return img_picto($langs->trans('PropalStatusOpened'), 'statut2') . ' ' . $this->labelstatut[$statut];
            if ($statut == 2)
                return img_picto($langs->trans('PropalStatusSigned'), 'statut4') . ' ' . $this->labelstatut[$statut];
            if ($statut == 3)
            ...
            if ($statut == 5)
                return img_picto($langs->trans('PropalStatusPoss'), 'statut3') . ' ' . $this->labelstatut[$statut];
        }
        if ($mode == 5) {
            if ($statut == 0)
                return $this->labelstatut_short[$statut] . ' ' . img_picto($langs->trans('PropalStatusDraftShort'), 'statut0');
            if ($statut == 1)
                return $this->labelstatut_short[$statut] . ' ' . img_picto($langs->trans('PropalStatusOpenedShort'), 'statut2');
            if ($statut == 2)
                return $this->labelstatut_short[$statut] . ' ' . img_picto($langs->trans('PropalStatusSignedShort'), 'statut4');
            if ($statut == 3)
            ...
            if ($statut == 5)
                return $this->labelstatut_short[$statut] . ' ' . img_picto($langs->trans('PropalStatusPossShort'), 'statut3');
        }
        if ($mode == 6) {
            if ($statut == 0)
                return img_picto($langs->trans('PropalStatusDraftShort'), 'statut0');
            ...
            if ($statut == 5)
                return img_picto($langs->trans('PropalStatusPossShort'), 'statut3');
            //return '<img src="'.DOL_URL_ROOT.'/theme/'.$conf->theme.'/img/statut'. $obj->id.'.png" border="0" alt="'.dol_escape_htmltag($obj->label).'" title="'.dol_escape_htmltag($obj->label).'">';
        }
    }
    ...
    function getIconList($url) {
        ...
        while ($i < $num) {
            ...
            $out.='<a href="' . $url . '&amp;status=' . $obj . '">';
            $out.= $this->LibStatut($obj, 6);
            $out.='</a>';
            $i++;
        }
...
class PropaleLigne {
    ...
    function insert($notrigger = 0) {
        ...
        $sql.= ' ' . $this->rang . ',';
        if (isset($this->marge_tx))
            $sql.= ' ' . $this->marge_tx . ',';
        else
            $sql.= ' null,';
        if (isset($this->marque_tx))
            $sql.= ' ' . $this->marque_tx;
        else
            $sql.= ' null';
        $sql.= ')';
