    ...
    if ($result >= 0 && isset($_POST['qty']) && (($_POST['np_price'] != '' && ($_POST['np_desc'] || $_POST['dp_desc'])) || $_POST['idprod'])) {
        ...
        if ($price_min && (price2num($pu_ht) * (1 - price2num($_POST['remise_percent']) / 100) < price2num($price_min))) {
            ...
            $result = $object->addline(
                    $_POST["id"], $desc, $pu_ht, $_POST['qty'], $tva_tx, $localtax1_tx, $localtax2_tx, $_POST['idprod'], $_POST['remise_percent'], $price_base_type, $pu_ttc, $info_bits, $type, $prod->ecotax, -1, 0, $_POST['fk_parent_line']
            );
...
if ($_POST['action'] == 'updateligne' && $user->rights->propale->creer && $_POST["save"] == $langs->trans("Save")) {
    ...
    if ($productid && $price_min && (price2num($up_ht) * (1 - price2num($_POST['remise_percent']) / 100) < price2num($price_min))) {
        ...
        $result = $object->updateline($_POST['lineid'], $up_ht, $_POST['qty'], $_POST['remise_percent'], $vat_rate, $localtax1_rate, $localtax2_rate, $_POST['desc'], 'HT', $info_bits, $product->ecotax, $special_code, $_POST['fk_parent_line']
        );
...
if ($id > 0 || !empty($ref)) {
    ...
    if ($conf->global->PROPALE_ADD_DELIVERY_ADDRESS) {
        ...
        if ($action != 'editdelivery_address' && $object->brouillon)
            print '<td align="right"><a href="' . $_SERVER["PHP_SELF"] . '?action=editdelivery_address&amp;socid=' . $object->socid . '&amp;id=' . $object->id . '">' . img_edit($langs->trans('SetDeliveryAddress'), 1) . '</a></td>';
        print '</tr></table>';
        print '</td><td colspan="3">';
    ...
    if ($conf->projet->enabled) {
        ...
        if ($user->rights->propale->creer) {
            if ($action != 'classer')
                print '<td align="right"><a href="' . $_SERVER['PHP_SELF'] . '?action=classer&amp;id=' . $object->id . '">' . img_edit($langs->trans('SetProject')) . '</a></td>';
            print '</tr></table>';
            print '</td><td colspan="3">';
            if ($action == 'classer') {
    ...
    if (!empty($object->lines))
        $object->printObjectLines($action, $mysoc, $soc, $lineid);
    ...
    $sql.= 'p.rowid as propalid, p.total_ht, p.ref_client as ref, p.fk_statut, p.fk_user_author, p.datep as dp, p.fin_validite as dfv,';
    ...
    if ($result) {
        ...
        while ($i < min($num, $limit)) {
            ...
            print $objectstatic->getNomUrl(1,'','',20);
            print '</td>';
