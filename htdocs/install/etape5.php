...
if (!isset($force_install_lockinstall))
	...
	if ($action == "set") {
		...
		if ($db->connected == 1) {
			...
			if (!empty($force_install_module)) {
				...
				if (!defined('DOL_DOCUMENT_ROOT_ALT') && !empty($dolibarr_main_document_root_alt))
					define('DOL_DOCUMENT_ROOT_ALT', $dolibarr_main_document_root_alt);
...
if ($action == "set") {
	if (empty($conf->global->MAIN_VERSION_LAST_UPGRADE) || ($conf->global->MAIN_VERSION_LAST_UPGRADE == DOL_VERSION)) {
		...
		if (!empty($force_install_lockinstall)) {
			...
			$fp = @fopen($lockfile, "w");
			if ($fp) {
				if ($force_install_lockinstall == 1)
